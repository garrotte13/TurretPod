...
function reloadPod.Init()
    ...
    weapons_equipment = storage.reloadPods.weapons_equipment
    equipped_weapons_count = storage.reloadPods.equipped_weapons_count
    equipped_weapon_id = storage.reloadPods.equipped_weapon_id
    equipped_weapon_last = storage.reloadPods.equipped_weapon_last

    grids = storage.reloadPods.grids
...
function reloadPod.AddWeapon(weapon, grid_id, untilTick)
    ...
    if r == 0 then
        ...
        storage.reloadPods.equipped_weapon_last = storage.reloadPods.equipped_weapon_last + 1
        r = storage.reloadPods.equipped_weapon_last
    end
    --weapon.energy = 0
    storage.reloadPods.weapons_equipment[r] = {
        weapon = weapon,
        type = weapon_type,
        ammo = weapon_ammo,
        ammo_count = 0,
...
function reloadPod.NewEquipment(weapon, grid)
    if weapon.type == "active-defense-equipment" and weapon.name:match("turret%-pod%-(.+)%-t%d") then
        ...
        for ids = 1 , storage.reloadPods.last_grid do
            if not storage.reloadPods.grids[ids] then
                if r == 0 then r = ids end
            elseif storage.reloadPods.grids[ids].grid == grid then
                grid_id = ids
                break
            end
        end
        if grid_id == 0 then
            if r == 0 then
                --game.print ("Last member of grids was removed incorrectly! Total grids count: " .. storage.reloadPods.grids_count .. ". First free grid order number: " .. storage.reloadPods.last_grid)
                storage.reloadPods.last_grid = storage.reloadPods.last_grid + 1
                r = storage.reloadPods.last_grid
            end
            grid_id = r
            storage.reloadPods.grids[grid_id] = {
                grid = grid,
                weapons = {},
                owner = nil,
                unit = 0,
    ...
    if weapon_name:match("turret%-pod%-(.+)%-t%d") then
        ...
        if grid_id > 0 then
            ...
            for _, weapon_id in pairs(storage.reloadPods.grids[grid_id].weapons) do
                if storage.reloadPods.weapons_equipment[weapon_id].weapon and storage.reloadPods.weapons_equipment[weapon_id].weapon.valid then
            ...
            if i > 0 then
                storage.reloadPods.grids[grid_id].weapons = to_keep
                for ids = 1, i do
                    --game.print("Removed " .. weapon_name .. " Pod index:" .. to_remove[ids])
                    storage.reloadPods.weapons_equipment[to_remove[ids]] = nil
                end
                storage.reloadPods.equipped_weapons_count = storage.reloadPods.equipped_weapons_count - i
            else --game.print("No turret pods at all were found to remove!")
            end
        else
            --game.print("Grid was not found! Can't remove turret pods from it.")
        end
        --game.print("Total amount of remaining pods in all active grids: " .. storage.reloadPods.equipped_weapons_count)
    end
end
...
function reloadPod.GridGetsOwner(entity, fast)
    ...
    if r and r > 0 then
        ...
        storage.reloadPods.grids[grid_id].unit = r
    end
    return grid_id
end
    ...
    if storage.reloadPods.grids[grid_id].weapons then
        for _, weapon_id in pairs(storage.reloadPods.grids[grid_id].weapons) do
            if storage.reloadPods.weapons_equipment[weapon_id] then
                --game.print("Removed: turret-pod-" .. storage.reloadPods.weapons_equipment[weapon_id].type .. "-t" .. storage.reloadPods.weapons_equipment[weapon_id].tier .. "-" .. storage.reloadPods.weapons_equipment[weapon_id].ammo .. "-equipment. Pod's index: " .. weapon_id)
                storage.reloadPods.weapons_equipment[weapon_id] = nil
                storage.reloadPods.equipped_weapons_count = storage.reloadPods.equipped_weapons_count - 1
...
function reloadPod.GridLosesOwnerEntity(grid) --should keep for char armor in the end
    ...
    if grid_id > 0 then
        if grid.equipment[1] then
            storage.reloadPods.grids[grid_id].owner = nil
            storage.reloadPods.grids[grid_id].unit = 0
           -- game.print("A grid lost its inventory connection, because it's owner was put in the box or dropped on the ground. Grid's index: " .. grid_id)
            ...
            if storage.reloadPods.grids[grid_id].weapons and storage.reloadPods.grids[grid_id].weapons[1] then
                --game.print("We've got a problem - lost pods in array")
            end
            storage.reloadPods.grids[grid_id] = nil
            storage.reloadPods.grids_count = storage.reloadPods.grids_count - 1
            --game.print("Grid is empty and lost inventory link - will be destroyed. It's index was: " .. grid_id .. " Grid count now is: " .. storage.reloadPods.grids_count)
        end
    end
end
    ...
    for _, vehicle in pairs(entities) do
        if vehicle and vehicle.valid and vehicle.grid and vehicle.get_inventory(defines.inventory.car_trunk) and vehicle.get_inventory(defines.inventory.car_trunk).valid then
            ...
            if grid_id > 0 then
                ...
                this_grid = storage.reloadPods.grids[grid_id]
                if this_grid.weapons and this_grid.weapons[1] then
                    for _, weapon_id in pairs(this_grid.weapons) do
                        if storage.reloadPods.weapons_equipment[weapon_id] and storage.reloadPods.weapons_equipment[weapon_id].weapon
                        then
                            this_pod = storage.reloadPods.weapons_equipment[weapon_id]
